...
import { deidentify, reattach, coarseLocation } from './deidentify.mjs';
...
async function readPdf(buffer) {
  ...
  for (let i = 1; i <= pageCount; i++) {
    // Two faults lived here, and every scanned CV hit the first one:
    //  - unpdf needs a canvas in Node and will not guess which one, so without
    //    canvasImport it threw "canvasImport is required";
    //  - getDocumentProxy hands `bytes` to the PDF.js worker as a transfer,
    //    which detaches the buffer, so a second parse from the same bytes died
    //    with "Cannot transfer object of unsupported type". Rendering from the
    //    document already open avoids the second parse altogether.
    // Both failed identically on retry and were gone from Vercel's hour of logs
    // by the time anyone wrote in. A scanned fixture now covers this path.
...
/**
 * The whole extraction, retries included, has to finish inside the serverless
 * function's life (maxDuration 60 in the route). One attempt used to be allowed
 * 90 seconds, so a stalled model call outlived the function: Vercel killed it,
 * the visitor saw a generic failure, and nothing was logged. Now each attempt
 * gets what is left of the budget, and a retry is only started if it could
 * plausibly finish.
 */
export const EXTRACT_BUDGET_MS = 48_000;
const ATTEMPT_MS = 30_000;
const MIN_RETRY_MS = 8_000;

export async function extract(buffer, filename, {
  apiKey = process.env.GEMINI_API_KEY, retries = 2, budgetMs = EXTRACT_BUDGET_MS,
} = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      const payload = await callGemini(parts, apiKey, AbortSignal.timeout(Math.max(1000, Math.min(ATTEMPT_MS, left))));
      ...
      // A scan has nothing to reattach, but its location still gets coarsened.
      const data = found
        ? reattach(parsed, found)
        : { ...parsed, location: coarseLocation(parsed.location) };
      data._source = text ? 'text' : 'image';
      // Kept so the caller can check how much of the CV survived. Never stored,
      // never logged - it lives only for the duration of this request.
      Object.defineProperty(data, '_sourceText', { value: text, enumerable: false });
