...
import { readableOnWhite } from './colour.mjs';
...
// ------------------------------------------------------------------ pieces
/**
 * Calibri on Venditas's own layout. Inside an agency's template (lib/template.mjs)
 * runs carry no font at all, so the template's own fonts apply.
 */
const fontOf = (font) => (font === null ? {} : { font: font || 'Calibri' });

const textPara = (content, opts = {}) =>
  new Paragraph({
...
const heading = (label, colour, font) =>
  new Paragraph({
...
/**
 * The logo, fitted inside 150 x 56 at its own proportions. The fixed 150 x 44
 * this replaced squashed every square logo to a third of its height, in the
 * document an agency judges the product by.
 */
function logoBox(buf, maxWidth = 150, maxHeight = 56) {
  const size = imageSize(buf);
  if (!size?.width || !size?.height) return { width: maxWidth, height: 44 };
  const scale = Math.min(maxWidth / size.width, maxHeight / size.height);
  return { width: Math.round(size.width * scale), height: Math.round(size.height * scale) };
}

const bulletPara = (content, font) =>
  new Paragraph({
    bullet: { level: 0 },
    spacing: { after: 40 },
    children: [new TextRun({ text: content, size: 19, ...fontOf(font) })],
  });
 ...
 * @param {object} options   { reference, template }. With template: true the
 *                           masthead, footer and fonts are left out, because
 *                           the agency's own template supplies them
 *                           (lib/template.mjs does the rest).
 * @returns {Promise<Buffer>} the .docx
 */
export async function render(data, brand = {}, { reference, template = false } = {}) {
  const colour = readableOnWhite(brand.colour);
  const redact = brand.redact !== false;
  const ref = reference || makeReference(data.name);
  const body = [];
  const F = template ? null : undefined;
  const text = (content, opts = {}) => textPara(content, { ...opts, font: F });
  const sectionHeading = (label, c) => heading(label, c, F);
  const bullet = (content) => bulletPara(content, F);

  // ---- masthead ---------------------------------------------------------
  // The template's header carries the agency's logo and name.
  if (template) {
    // nothing here
  } else if (brand.logo) {
    body.push(
      new Paragraph({
        ...
        children: [
          new ImageRun({
            data: brand.logo,
            transformation: logoBox(brand.logo),
  ...
  // ---- anything the fixed schema has no slot for -------------------------
  // Rendered before the flat lists because it is usually substantive content —
  // a solicitor's matters, a nurse's audits — not a trailing list.
  for (const section of data.other_sections || []) {
    if (!section?.items?.length) continue;
    body.push(sectionHeading(section.heading || 'Additional', colour));
    for (const item of section.items) body.push(bullet(item));
  }

  // ---- flat lists -------------------------------------------------------
  for (const [label, key] of [['Skills', 'skills'], ['Languages', 'languages']]) {
  ...
  // Certifications carry a date and issuer, so they cannot be joined with a
  // separator like the flat lists above without losing exactly the detail that
  // makes them worth listing.
  const certs = data.certifications || [];
  if (certs.length) {
    body.push(sectionHeading('Certifications', colour));
    for (const c of certs) {
      const name = typeof c === 'string' ? c : c?.name;
      if (!name) continue;
      const detail = typeof c === 'string'
        ? ''
        : [c.issuer, c.date].filter(Boolean).join(', ');
      body.push(text(detail ? `${name} — ${detail}` : name, { size: 9.5, after: 2 }));
    }
  }
  ...
  const avBits = [
    av.notice_period && `Notice: ${av.notice_period}`,
    av.right_to_work && `Right to work: ${av.right_to_work}`,
    av.salary_expectation && `Salary expectation: ${av.salary_expectation}`,
  ].filter(Boolean);
  if (avBits.length) {
    body.push(sectionHeading('Availability', colour));
    body.push(text(avBits.join('  ·  '), { size: 9.5, after: 2 }));
  }
  ...
  // The template's footer replaces this one.
  if (!template) {
    section.footers = {
      default: new Footer({
        children: [
