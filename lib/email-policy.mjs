 ...
 *
 * Nine layers, cheapest first, network last. Each one returns a reason a person
 * can act on, because every rejection here happens in front of a prospect who
 * has already spent half an hour on a call, and "invalid email" is an insult
 * when the real answer is "we need your work address, not your Gmail".
 *
...
/**
 * Consumer mailboxes. Not fraudulent — but this is a business tool sold to
 * recruitment agencies, the form has always said "work email", and a free
 * provider is where address-aliasing actually happens: one Gmail account is an
 * unlimited supply of alice+1, alice+2, a.l.i.c.e.
 *
 * This is the single most effective control in the file and also the one most
 * likely to reject a real customer, because a two-person agency may genuinely
 * run on Gmail. That is why the rejection says how to get past it — reply and
 * ask — rather than pretending the address is invalid.
 */
