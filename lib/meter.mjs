...
/**
 * What a verified demo account gets: five CVs, then the conversation about
 * paying. Decision 012.
 *
 * Five is enough to see the output on their own candidates and their own
 * branding, which is the only question a demo has to answer, and not enough to
 * clear a shortlist with — so the next CV is a reason to talk rather than a
 * reason to keep going quietly. The earlier fifty optimised for habit; that is
 * the right instinct for a product with a checkout and the wrong one for a
 * business that closes by invoice inside a month.
 *
 * Per-day is the same five: a total of five makes any daily figure above it
 * decorative, and two numbers that cannot both bind is one number too many.
 */
export const DEMO_TOTAL = 5;
export const DEMO_PER_DAY = 5;
 ...
 */
export async function check(request, email, { account = null } = {}) {
  ...
  // A verified demo account meters against the address that was confirmed by
  // clicking a link, not the one typed into this form. Otherwise one account
  // cookie plus a fresh address each time is the alias loophole again, wearing
  // a different hat.
  const perDay = account ? DEMO_PER_DAY : FREE_PER_DAY;
  const total = account ? DEMO_TOTAL : FREE_TOTAL;
  const canon = account ? canonicalEmail(account) : (email ? canonicalEmail(email) : '');
  ...
  try {
    ...
    if (c.emailToday > perDay) {
      return { allow: false, reason: 'daily-cap', needEmail: false, used: c.emailToday, limit: perDay };
    }
    if (c.emailTotal > total) {
      return { allow: false, reason: 'trial-used', needEmail: false, used: c.emailTotal, limit: total };
    }

    return {
      allow: true,
      reason: account ? 'demo-account' : 'free-trial',
      needEmail: false, used: c.emailTotal, limit: total,
    };
  } catch (e) {
...
/**
 * Count a failed run, by reason, against the same hashed key the trial is
 * metered on. Reuses usage_daily and bump_usage (sql/001): no migration, no
 * content, purged on the same 90-day clock. Vercel keeps an hour of logs on
 * the free plan, and "it failed" arrives days later; this is what survives.
 * Never throws — a failure to count a failure is not worth a second failure.
 */
export async function recordFailure(email, reason) {
  if (!metered) return;
  const key = email ? emailKey(email) : 'anon';
  const kind = `fail:${String(reason || 'unknown').replace(/[^a-z0-9-]/gi, '-').slice(0, 40)}`;
  try {
    await bump(key, kind);
  } catch (e) {
    console.error('failure count failed:', e.message);
  }
}
